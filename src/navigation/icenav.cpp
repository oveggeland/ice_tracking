...
void IceNav::gnssCallback(const sensor_msgs::NavSatFix::ConstPtr& msg){
    ...
    if (init_){
        ...
        auto altitude_factor = LeveredAltitudeFactor(X(correction_count_), L(0), noiseModel::Isotropic::Sigma(1, 0.1));
        ...
        if (correction_count_ % 10 == 0){
            LevenbergMarquardtOptimizer optimizer(graph_, values_);
            values_ = optimizer.optimize();
        }

        if (correction_count_ == 250){
            finish();
        }

        // Reset IMU preintegration
        imu_handle_.resetIntegration(ts, values_.at<imuBias::ConstantBias>(B(correction_count_-1)));
        ...
        // Add initial values
        ROS_WARN_STREAM("Initial rot is " << imu_handle_.getPriorRot().rpy().transpose());
        ...
        values_.insert(L(0), Point3(20, 0, 0));
