    ...
    for (const std::string& filename : files) {
        ...
        for (rosbag::MessageInstance const m : view) {
            if (m.getDataType() == "sensor_msgs/Imu") {
                ...
                auto imu_callback = std::function<void()>(std::bind(&IceTrack::imuCallback, &tracker, msg));
                ...
                auto gnss_callback = std::function<void()>(std::bind(&IceTrack::gnssCallback, &tracker, msg));
                sequencer.pushCallback(msg->header.stamp, gnss_callback);
            }
            else if (m.getDataType() == "sensor_msgs/PointCloud2") {
                sensor_msgs::PointCloud2::ConstPtr msg = m.instantiate<sensor_msgs::PointCloud2>();

                auto pcl_callback = std::function<void()>(std::bind(&IceTrack::pclCallback, &tracker, msg));
