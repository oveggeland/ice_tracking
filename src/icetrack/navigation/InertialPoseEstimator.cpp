#include "icetrack/navigation/InertialPoseEstimator.h"

// Constructors
InertialPoseEstimator::InertialPoseEstimator(){}

InertialPoseEstimator::InertialPoseEstimator(ros::NodeHandle nh, std::shared_ptr<SensorSystem> system){
    // Submodules
    imu_integration_ = ImuIntegration(nh, system);
    gnss_correction_ = GnssCorrection(nh, system);
    surface_estimation_ = SurfaceEstimation(nh, system);
...
bool InertialPoseEstimator::imuUpdate(){
    if (init_){
        imu_integration_.integrate();
        if (imu_integration_.timeOut()){
            ROS_WARN("InertialPoseEstimator: IMU timeout");
...
bool InertialPoseEstimator::lidarUpdate(){
    return false; // Nothing to do here
};

Pose3 InertialPoseEstimator::getPose() const{
    return pose_;
}

void InertialPoseEstimator::initialize(double ts){
    // Initial state
    ts_ = ts;
...
*/
void InertialPoseEstimator::newCorrection(double ts){
...
void InertialPoseEstimator::writeToFile(){
    f_nav_ << ts_ << ",";
